...
constexpr float DAC_VCC = 3300.0;
...
constexpr float CURRENT_SENSE_OFFSET = OPAMP_VCC * (1000.0 / 3451000.0);                      // R1 =3.3 megaohm R2 =1k ohm vcc 5 volts val = !1.515 mv
constexpr float CURRENT_SET_OFFSET = OPAMP_VCC * (1000.0 / 1001000.0);                        // R1 = 1megaohm R2= 1k   val = ~ 5 mv
constexpr float VOUT_SENSE_OFFSET = (OPAMP_VCC * (100.0 / (3100.0))) * (100000.0 / 105100.0); // val = ~ 153 mv , out- is 30 mv above ground
constexpr float VOUT_SENSE_DIVIDER_RATIO = (5200.0 / 105200.0);                               // val =~ 0.0494;
constexpr float VOLTAGE_SET_GAIN = 1 + (5100.0 / 99000.0);                                    // APPROXI = 1.51
class PSU
{
private:
    float _currentGain = 28.0f;
    float _current_shuntResistor = 3.333f;
    Adafruit_MCP4725 _DAC_V;
    Adafruit_MCP4725 _DAC_A;
    Adafruit_ADS1115 _ADS;
    float _V_set = 0;
    float _A_set = 0;
    uint32_t _DAC_A_steps = 0;
    uint16_t _DAC_V_steps = 0;
